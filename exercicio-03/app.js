...
const foodsInfo = 'Até aqui, o array "brazilianFoods" possui '
    +`${brazilianFoods.length} itens: ${brazilianFoods.join(', ')}.`
...
const dessert = `${dogNames[0].slice(dogNames[0].length - 2, dogNames[0].length)}${dogNames[1].slice(dogNames[1].length - 2, dogNames[1].length)}${dogNames[3].slice(dogNames[3].length - 2, dogNames[3].length)}`.replace('n', 'd')
const sobremesa = `A ${dessert} é um doce à base de coco, tradicional na América Latina e em Angola.`
